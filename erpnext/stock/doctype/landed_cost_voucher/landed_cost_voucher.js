...
erpnext.stock.LandedCostVoucher = erpnext.stock.StockController.extend({
	...
	set_applicable_charges_for_item: function() {
		...
		if(this.frm.doc.taxes.length) {
			...
			$.each(this.frm.doc.taxes, function(i, t) {
				$.each(this.frm.doc.items || [], function(i, item) {
					applicable_charges = flt(item[based_on]) * flt(t.amount) / flt(total_item_cost)
					item.applicable_charges += flt(applicable_charges, precision("applicable_charges", item))
					total_charges += item.applicable_charges

					item.account_wise_charges[t.expense_account] = item.applicable_charges;
				});
